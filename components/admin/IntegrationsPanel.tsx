...
export default function IntegrationsPanel() {
  ...
  useEffect(() => {
    ...
    // Detect OAuth result param injected by the callback redirect
    ...
    if (oauthResult === 'connected') {
      ...
      setNotice({ message: 'Google OAuth failed. Check your Client ID / Secret and try again.', tone: 'error' });
    } else if (oauthResult === 'missing_client') {
      setNotice({ message: 'Save your Google Client ID and Client Secret first, then connect the owner account.', tone: 'error' });
    }
    if (oauthResult) {
  ...
  // Show "connected" if we have an email OR a refresh token — email may be missing if
  // getGoogleOAuthEmail() failed on first login but the token was still stored.
    ...
    <div data-testid="integrations-panel" className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
          <div className={`rounded-lg border p-4 lg:col-span-2 transition-colors duration-500 ${isGoogleConnected ? 'border-emerald-300 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  ...
                  {isGoogleConnected ? (
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 border border-emerald-300 px-3 py-0.5 text-xs font-semibold text-emerald-700">
                      <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                      Connected
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 border border-zinc-300 px-3 py-0.5 text-xs font-semibold text-zinc-500">
                ...
                {isGoogleConnected ? (
                  <p className="mt-3 text-xs font-medium text-emerald-700">
                    ✓ {activeConfig.connectedGoogleEmail || 'Google account'} · Token: {activeConfig.secrets.refreshToken.isSet ? activeConfig.secrets.refreshToken.maskedValue : '—'} · Connected: {formatDate(activeConfig.oauthConnectedAt)}
                  </p>
                ) : (
                  <p className="mt-2 text-xs text-zinc-500">
                    Refresh token: not stored. Connected at: Never.
                  </p>
                )}
              </div>
              <button
                ...
                className={`shrink-0 rounded-lg px-5 py-2.5 text-sm font-medium text-white shadow-sm transition-colors ${isGoogleConnected ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-zinc-900 hover:bg-amber-600'}`}
              >
                {isGoogleConnected ? 'Reconnect Google Owner' : 'Connect Google Owner'}
              </button>
            </div>
          </div>
