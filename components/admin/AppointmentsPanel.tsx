...
export default function AppointmentsPanel() {
  ...
  const loadAppointments = async () => {
    ...
    try {
      const response = await fetch('/api/admin/appointments?limit=150', {
        cache: 'no-store',
      });
      ...
      setNotice({ message: 'Appointments refreshed.', tone: 'success' });
    } catch (error) {
  ...
  const stats = useMemo(() => {
    return records.reduce(
      ...
      { total: 0, confirmed: 0, conflicts: 0, failed: 0 },
    );
  }, [records]);
    ...
    <div data-testid="appointments-panel" className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div className="flex flex-col gap-4 border-b border-zinc-100 pb-5 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">
              Review contact form appointments, Calendar event links, email delivery status, and errors without exposing credentials.
            </p>
          </div>
          <button
      ...
      <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
        {loading && records.length === 0 ? (
          ...
          <div className="p-8 text-sm text-zinc-500">No appointment requests have been recorded yet.</div>
        ) : (
