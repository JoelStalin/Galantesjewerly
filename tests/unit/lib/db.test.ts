...
describe('CMS persistence', () => {
  ...
  it('preserves persisted logo urls while merging settings defaults', async () => {
    const localCms = {
      settings: {
        site_title: 'Local title',
        site_description: 'Local description',
  ...
  it('migrates legacy featured image assets to managed image URLs', async () => {
    const localCms = {
      settings: {
        site_title: 'Local title',
        site_description: 'Local description',
      ...
      featured_items: [
        {
          ...
          content_text: 'Legacy image asset',
          image_url: '/assets/images/section-bridal.png',
        ...
        },
        {
          ...
          content_text: 'Legacy image asset',
          image_url: '/assets/images/section-collections.png',
        ...
        },
        {
          ...
          title: 'Master Repair',
          content_text: 'Legacy image asset',
          image_url: '/assets/images/featured-repairs.png',
    ...
    const { getFeaturedItems } = await import('@/lib/db');
    const featured = await getFeaturedItems();

    expect(featured.map((item) => item.image_url)).toEqual([
    ...
    expect(JSON.parse(fileStore.get(cmsFilePath)!)).toMatchObject({
      featured_items: [
        expect.objectContaining({
