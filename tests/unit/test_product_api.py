...
class FakeGalleryImage:
    def __init__(self, image_id, sequence=1, alt_text='', active=True):
        self.id = image_id
...
class FakeRequest:
    def __init__(self, env, headers=None):
        ...
        self.httprequest = types.SimpleNamespace(
            host_url='https://fallback.example.com/',
            headers=headers or {},
        )
...
class ProductAPITestCase(unittest.TestCase):
    ...
    def test_resolve_base_url_upgrades_public_http_to_https(self):
        self.config_model = FakeConfigParamModel(base_url='http://odoo.galantesjewelry.com')
        env = FakeEnv({
            'product.template': self.product_model,
            'product.category': self.category_model,
            'ir.config_parameter': self.config_model,
        ...
        controller = self.module.ProductAPIController()
        self.assertEqual(controller._resolve_base_url(), 'https://odoo.galantesjewelry.com')

    def test_serialize_product_includes_sorted_gallery_images(self):
        self.uncategorized_product.gallery_ids = [
            FakeGalleryImage(33, sequence=30, alt_text='Third image'),
            FakeGalleryImage(11, sequence=10, alt_text='First image'),
            FakeGalleryImage(22, sequence=20, alt_text='Second image'),
            FakeGalleryImage(44, sequence=40, alt_text='Hidden image', active=False),
        ]
        ...
        self.assertEqual(
            payload['gallery'],
            [
                'https://shop.galantesjewelry.com/api/products/gallery-image/11',
                'https://shop.galantesjewelry.com/api/products/gallery-image/22',
                'https://shop.galantesjewelry.com/api/products/gallery-image/33',
            ],
        )
        self.assertEqual([item['sequence'] for item in payload['galleryImages']], [10, 20, 30])
        self.assertEqual([item['altText'] for item in payload['galleryImages']], ['First image', 'Second image', 'Third image'])

    def test_image_routes_use_deterministic_fallback_payload(self):
        controller = self.module.ProductAPIController()
        payload = controller._fallback_image_payload()

        self.assertEqual(payload['mimetype'], 'image/svg+xml; charset=utf-8')
        self.assertIn(b"Galante's Jewelry", payload['content'])
